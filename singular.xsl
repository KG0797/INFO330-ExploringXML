...
Single type pokemon: <xsl:value-of select="count(pokemon[type])" />:

<xsl:apply-templates select="pokemon[type and count(type)=1]" />
</xsl:template>
...
<xsl:template match="/pokedex">
  ...
  Count: <xsl:value-of select="count(pokemon[type and not(type[2])])" />
  <table border="1">
    <tr bgcolor="#9acd32">
      <th>Name</th>
    ...
    <xsl:apply-templates select="pokemon[type and not(type[2])]" />
  </table>
  </body>
  </html>
</xsl:template>
